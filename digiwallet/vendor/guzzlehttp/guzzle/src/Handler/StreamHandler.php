...
class StreamHandler
{
    ...
    public function __invoke(RequestInterface $request, array $options)
    {
        ...
        try {
            ...
            return \DigiwalletGuzzleHttp\Promise\rejection_for($e);
        }
    }
    ...
    private function createResponse(
        ...
        $headers = \DigiwalletGuzzleHttp\headers_from_lines($hdrs);
        ...
        if (isset($options['on_headers'])) {
            try {
                ...
                return \DigiwalletGuzzleHttp\Promise\rejection_for($ex);
            }
        }
    ...
    private function checkDecode(array $options, array $headers, $stream)
    {
        ...
        if (!empty($options['decode_content'])) {
            $normalizedKeys = \DigiwalletGuzzleHttp\normalize_header_keys($headers);
    ...
    private function add_proxy(RequestInterface $request, &$options, $value, &$params)
    {
        if (!is_array($value)) {
            ...
            if (isset($value[$scheme])) {
                if (!isset($value['no'])
                    || !\DigiwalletGuzzleHttp\is_host_in_noproxy(
    ...
    private function add_verify(RequestInterface $request, &$options, $value, &$params)
    {
        if ($value === true) {
            ...
            if (PHP_VERSION_ID < 50600) {
                $options['ssl']['cafile'] = \DigiwalletGuzzleHttp\default_ca_bundle();
            }
    ...
    private function add_debug(RequestInterface $request, &$options, $value, &$params)
    {
        ...
        $value = \DigiwalletGuzzleHttp\debug_resource($value);

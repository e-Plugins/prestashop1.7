...
{
    const APP_ID = 'dw_prestashop1.7_1.1.2';
...
| No rtlo (layoutcode Digiwallet) filled in, check the module settings";
    ...
    public function startPayment()
    {
        ...
        $url .= "&rtlo=" . urlencode($this->rtlo) . "&amount=" . urlencode($this->amount) . "&description=" .
             urlencode($this->description) . "&userip=" . urlencode($_SERVER["REMOTE_ADDR"]) . "&domain=" .
             urlencode($_SERVER["HTTP_HOST"]) . "&app_id=" . urlencode(self::APP_ID) . "&returnurl=" .
             urlencode($this->returnUrl) .
             ((! empty($this->cancelUrl)) ? "&cancelurl=" . urlencode($this->cancelUrl) : "") . "&reporturl=" .
             urlencode($this->reportUrl);
    ...
    public function checkPayment($transactionId)
    {
        ...
        $url = $this->checkAPIs[$this->payMethod] . "?" . "rtlo=" . urlencode($this->rtlo) . "&" . "trxid=" .
             urlencode($transactionId) . "&" . "&once=0";

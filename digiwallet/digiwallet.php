<?php
/**
 * Activates iDeal, Bancontact, Sofort, Visa / Mastercard Credit cards, PaysafeCard, AfterPay, BankWire, PayPal
 * and Refunds in Prestashop.
 *
...
class Digiwallet extends PaymentModule
{
    ...
    public function __construct()
    {
        ...
        $this->limited_currencies = [
            self::DIGIWALLET_CURRENCY,
        ];
        $this->bootstrap = true;
...
BankWire, PayPal and Refunds in Prestashop'
    ...
    public function install()
    {
        ...
        if (!parent::install() || !$this->createDigiwalletTable() || !$this->updateDigiwalletTable() ||
             !$this->createDigiwalletStatus() || !$this->registerHook('displayHeader') ||
             !$this->registerHook('displayBackOfficeHeader') || !$this->registerHook('paymentOptions') ||
             !$this->registerHook('paymentReturn') || !$this->registerHook('actionAdminControllerSetMedia') ||
    ...
    public function getContent()
    {
        ...
        if (Tools::isSubmit('submit'.$this->name)) {
            ...
            if (!$RTLO || empty($RTLO) || !Validate::isGenericName($RTLO) || !Validate::isUnsignedInt($RTLO)) {
                ...
                Configuration::updateValue('BANK_LIST_MODE', (1 == $bankListMode) ? '1' : '0');
                Configuration::updateValue('COUNTRY_LIST_MODE', (1 == $countryListMode) ? '1' : '0');
    ...
    private function displayForm()
    {
        ...
        $listMethods = $this->getListMethods();
        foreach ($listMethods as $id => $method) {
            $helper->fields_value['DW_ENABLE_METHOD_'.$id] = 'yes' == $method['enabled'] ? 1 : 0;
    ...
    private function getConfigForm()
    {
        $arrInputs = [
            ...
            [
                'col' => 3,
                'type' => 'text',
                'desc' => $this->l('Go to: https://www.digiwallet.nl/en/user/dashboard').' >> '.
    ...
    private function checkCurrency($cart)
    {
        ...
        if (true == in_array($currencyOrder->iso_code, $this->limited_currencies)) {
            return true;
        }
    ...
    public function hookPaymentOptions($params)
    {
        ...
        foreach ($listMethods as $id => $method) {
            ...
            $addInfo = '';
            if (('IDE' == $id && 1 != Configuration::get('BANK_LIST_MODE')) ||
                 ('DEB' == $id && 1 != Configuration::get('COUNTRY_LIST_MODE'))) {
    ...
    public function updateOrderAfterCheck($transactionInfoArr, $isReport = false)
    {
        ...
        $amountPaid = null;
        $listMethods = $this->getListMethods();
        if (in_array($paymethod, [self::DIGIWALLET_EPS_METHOD, self::DIGIWALLET_GIP_METHOD])) {
        ...
        if ($isSuccess) {
            ...
            if (false !== strpos($errorMessage, 'DW_SE_0021')) {
    ...
    {
        if ('AdminOrders' == Tools::getValue('controller') && $id_order = Tools::getValue('id_order')) {
            $sql = sprintf(
...
AND paid_amount > 0",
            ...
            if ($row && !in_array($row['paymethod'], [self::DIGIWALLET_EPS_METHOD, self::DIGIWALLET_GIP_METHOD])) {
    ...
    public function validateOrder(
        ...
        if ('id_address_delivery' == Configuration::get('PS_TAX_ADDRESS_TYPE')) {
        ...
        if (Validate::isLoadedObject($this->context->cart) && false == $this->context->cart->OrderExists()) {
            if (false !== $secure_key && $secure_key != $this->context->cart->secure_key) {
                PrestaShopLogger::addLog(
                ...
                if (($rule = new CartRule((int) $cart_rule['obj']->id)) && Validate::isLoadedObject($rule)) {
                    if ($error = $rule->checkValidity($this->context, true, true)) {
                        ...
                        if (isset($this->context->cookie) && isset($this->context->cookie->id_customer) &&
                            ...
                            $error = $this->trans(
                                'The cart rule named "%2s" (ID %1s) used in this cart is not valid and has been 
withdrawn from cart',
                                [
                                    (int) $rule->id,
                                    $rule_name,
            ...
            foreach ($order_detail_list as $key => $order_detail) {
                ...
                if (isset($order->id)) {
                    ...
                    foreach ($order->product_list as $product) {
                        ...
                        $product_price = PS_TAX_EXC == Product::getTaxCalculationMethod() ?
                    ...
                    foreach ($cart_rules as $cart_rule) {
                        ...
                        if (1 == count($order_list) &&
                             $values['tax_incl'] > ($order->total_products_wt - $total_reduction_value_ti) &&
                             1 == $cart_rule['obj']->partial_use && $cart_rule['obj']->reduction_amount > 0) {
                        ...
                        $cart_rules_list[] = [
                            ...
                            'voucher_reduction' => (0.00 != $values['tax_incl'] ? '-' : '').
        ...
        if ($order->current_state != Configuration::get('PS_OS_ERROR') &&
            ...
            foreach ($order->getProducts() as $product) {
                ...
                if (isset($customized_datas[$product['id_product']][$product['product_attribute_id']])) {
                    $product_var_tpl['customization'] = [];
                    $custom_pa = $customized_datas[$product['id_product']][$product['product_attribute_id']];
                    foreach ($custom_pa[$order->id_address_delivery] as $customization) {
                        $customization_text = '';
                        if (isset($customization['datas'][Product::CUSTOMIZE_TEXTFIELD])) {
                            foreach ($customization['datas'][Product::CUSTOMIZE_TEXTFIELD] as $text) {
                                $customization_text .= '<strong>'.$text['name'].'</strong>: '.$text['value'].
                                     '<br />';
                            }
                        }
            ...
            $data = [
                ...
                '{total_products}' => Tools::displayPrice(
                    PS_TAX_EXC == Product::getTaxCalculationMethod() ?
